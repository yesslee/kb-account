<template>
    <div class="list-group border-0 mx-auto" style="width: 70%;">
    <a href="#" class="list-group-item list-group-item-action d-flex gap-3 py-2 border-0" aria-current="true">
      <div class="d-flex gap-3 w-100 justify-content-between align-items-center">
        ...
        <div class="d-flex align-items-center">
          <small class="fs-6 fw-bold" style="color: rgba(0, 0, 0, 0.7);">{{formatPrice(transactionItem.price)}}원</small>
        </div>
      </div>
    </a>
  </div>
  ...
  function formatPrice(value) {
    let formattedPrice = Number(value).toLocaleString();
    
    // 가격에 +/- 기호 붙이기
    if (value >= 0) {
      formattedPrice = `+${formattedPrice}`;
    } else {
      formattedPrice = `-${formattedPrice.slice(1)}`; // 음수 값의 경우 '-' 기호만 붙이기
    }
